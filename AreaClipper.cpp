...
// libarea's own types are not in a namespace, and Clipper2Lib has a Point of
// its own, so a using-directive here would make the bare name ambiguous. Name
// what this file actually uses instead.
...
using Clipper2Lib::SimplifyPath;
...
// through Area. Coordinates stay far inside Clipper's range: a kilometre
// expressed in millimetres is 1e13 here.
...
static void AddVertex(const CVertex& vertex, const CVertex* prev_vertex)
{
    if (vertex.m_type == 0 || prev_vertex == NULL) {
        ...
        // FitCircles produces and only when it is allowed to. Everywhere else
        // two coincident points mean a corner that does not turn -- MakeLoop
        // makes one at every straight-through vertex -- and a circle there
        // would be a blob the shape never had.
...
static void SetFromResult(CCurve& curve, Path64& p, bool reverse = true, bool is_closed = true)
{
    ...
    for (unsigned int j = 0; j < p.size(); j++) {
        ...
        CVertex vertex(
            0,
            Point(dp.x / CArea::m_units, dp.y / CArea::m_units),
            Point(0.0, 0.0)
        );
        if (reverse) {
...
// are exposed because the operations below hand back a flat list of contours,
...
void CurveToClipperPath(const CCurve& curve, Path64& path, bool reverse)
{
    MakePoly(curve, path, reverse);
}

void CurveFromClipperPath(CCurve& curve, Path64& path, bool reverse, bool is_closed)
{
    SetFromResult(curve, path, reverse, is_closed);
}

void CArea::Subtract(const CArea& a2, FillRule subject_fill, FillRule clip_fill)
{
    Clip(ClipType::Difference, a2, subject_fill, clip_fill);
}
...
void CArea::Union(const CArea& a2, FillRule subject_fill, FillRule clip_fill)
{
    Clip(ClipType::Union, a2, subject_fill, clip_fill);
}
...
void UnFitArcs(CCurve& curve)
{
    ...
    for (std::list<PointD>::iterator It = pts_for_AddVertex.begin(); It != pts_for_AddVertex.end();
        ...
        CVertex vertex(
            0,
            Point(pt.x / CArea::m_units, pt.y / CArea::m_units),
            Point(0.0, 0.0)
        );
        curve.m_vertices.push_back(vertex);
    }
